...
max <- 0.975
#実測値のデータ
ds <- read.csv("simulation_yakitori_data.csv")

N_mcmc <- length(ms$lp__)
alld_mcmc <- NULL
#pHデータの取得
dpH <- read.csv("pH_yakitori_data.csv")
...
plot(alld_mcmc[,1], alld_mcmc[,4], type="l", xlab="Model digestion duration (min)", ylab="Survival ratio (log N/N0)",
     lwd=3, cex.lab=2, cex.axis=1.6, ylim=c(-6.2, 0), xlim=c(0,180), yaxt="n")
axis(2,at=seq(-6,0,1),las=1,cex.lab=2,cex.axis=1.6, lwd=2)
