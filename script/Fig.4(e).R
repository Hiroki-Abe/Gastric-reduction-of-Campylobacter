...
max <- 0.975
#実測値のデータ
ds <- read.csv("simulation_lettuce_data.csv")

N_mcmc <- length(ms$lp__)
alld_mcmc <- NULL
#pHデータの取得
dpH <- read.csv("pH_lettuce_data.csv")
...
points(ds$TIME[1:3], ds$MEAN[1:3], cex=2.5, lwd=3)
arrows(ds$TIME[1:3],ds$MEAN[1:3],ds$TIME[1:3],ds$MEAN[1:3]+ds$STDEV[1:3],angle=90,length=0.1, lwd=3)
arrows(ds$TIME[1:3],ds$MEAN[1:3],ds$TIME[1:3],ds$MEAN[1:3]-ds$STDEV[1:3],angle=90,length=0.1, lwd=3)
points(ds$TIME[4:5], ds$MEAN[4:5], cex=4.5, pch="*", lwd=3)
