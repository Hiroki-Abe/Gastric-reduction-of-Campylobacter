...
max <- 0.975
#実測値のデータ
ds <- read.csv("simulation_mix_data.csv")

N_mcmc <- length(ms$lp__)
alld_mcmc <- NULL
#pHデータの取得
dpH <- read.csv("pH_mix_data.csv")
...
points(ds$TIME[1:6], ds$MEAN[1:6], cex=2.5, lwd=3)
arrows(ds$TIME[1:6],ds$MEAN[1:6],ds$TIME[1:6],ds$MEAN[1:6]+ds$STDEV[1:6],angle=90,length=0.1, lwd=3)
arrows(ds$TIME[1:6],ds$MEAN[1:6],ds$TIME[1:6],ds$MEAN[1:6]-ds$STDEV[1:6],angle=90,length=0.1, lwd=3)
points(ds$TIME[7], ds$MEAN[7], cex=4.5, pch="*", lwd=3)
